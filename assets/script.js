...
const run_control = (idx) => {
  ...
  // console.log(control_set_dk[idx]);

  // console.log(idx);
  item = control_set_dk[idx];
...
Array.from(
  ...
  if (0 < answer_count < 50) {
    ...
    console.log("word for idx", control_set_dk[answer_count].word);

    let is_correct;
    if (control_set_dk[answer_count].translation === user_answer) {
      is_correct = 1;
    } else {
      is_correct = 0;
    }
    let new_control_entry = {
      id: participantIDnow,
      word: control_set_dk[answer_count].word,
      translation: control_set_dk[answer_count].translation,
      answer: user_answer,
      correct: is_correct,
    };
...
end_form_submit_button.addEventListener("click", (e) => {
  ...
  experiment_form.forEach((observation) => {
    if ((observation.language === "en") & (observation.type === "word")) {
      let match_control_idx = control_set_dk
        .map((elm) => elm["word"])
